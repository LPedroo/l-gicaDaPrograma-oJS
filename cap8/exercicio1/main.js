...
function verificarVisita() {
  if (!localStorage.getItem("clube")) {
    alert("Seja bem-vindo! Está é a sua primeira visita ao nosso site");
  } else if (sessionStorage.getItem("visitas")) {
    // Verifica se já existe uma contagem na sessionStorage
    // Se existe, incrementa a contagem
    var count = parseInt(sessionStorage.getItem("visitas")) + 1;
    sessionStorage.setItem("visitas", count);
  } else {
    // Se não existe, cria uma contagem inicial
    sessionStorage.setItem("visitas", 1);
  }

  // Obtém a contagem atual e exibe no console (você pode exibir em outro lugar, como no HTML)
  var currentVisits = sessionStorage.getItem("visitas");
  alert(
    `Que bom que você voltou! Está é a sua visita de número ${currentVisits} ao nosso site.`
  );
}
