...
function inserirLinha(tabela, titulo, genero) {
  var linha = tabela.insertRow(-1); //Adiciona uma linha tabela
  ...
  col3.innerHTML = "<input type='checkbox'>"; //innerHTML renderiza o código
}
...
function gravarFilme(titulo, genero) {
  ...
  if (localStorage.getItem("filmesTitulo")) {
    ...
    var filmesTitulo = `${localStorage.getItem("filmeTitulo")}; ${titulo}`;
    var filmesGenero = `${localStorage.getItem("filmesGenero")}; ${genero}`;

    localStorage.setItem("filmesTitulo", filmesTitulo); // grava dados
    localStorage.setItem("filmesGenero", filmesGenero); // no LocalStorage
  } else {
...
function removerFilmes() {
  ...
  // percorre campos input type checkbox da tabela (exceto "Todos" no título por isso inicia em 1)
  for (var i = 1; i < ckExcluir.length; i++) {
